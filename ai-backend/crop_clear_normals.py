"""
YOLO Cell Cropper - Cropping clear normal cells.
Reads raw blood smear images, detects cells via YOLO, and saves
each crop directly to the normal folder with a unique UUID filename.
"""
import os
import cv2
import uuid
...
def main():
    input_dir = "Clear"
    output_dir = os.path.join("dataset", "train", "Normal")
    ...
    print(f"\n[SUCCESS] Cropped {total_crops} normal cells from {len(images)} images.")
