"""
LabMind AI — V1 Rebuild: 4-Class RBC Classifier Training Script

Trains CellClassifierCNN with 4 classes: normal, sickle, target, other_abnormal.
Uses ImageFolder with forced class mapping to match V1Provider's RBC_CLASS_MAP_4.

Usage:
    cd ai-backend
    .\.venv\Scripts\Activate.ps1
    python train_classifier_v1.py

Output:
    cell_classifier_v1_rebuild.pth  (auto-detected by V1Provider on next restart)
"""
...
# ── CNN Architecture (must match V1Provider exactly) ──
...
# ── Forced class mapping (must match V1Provider.RBC_CLASS_MAP_4) ──
# V1Provider expects: {0: "normal", 1: "sickle", 2: "target", 3: "other_abnormal"}
TARGET_CLASS_TO_IDX = {
    "normal": 0,
    "sickle": 1,
    "target": 2,
...
def main():
    ...
    model_save_path = "cell_classifier_v1_rebuild.pth"
    batch_size = 32
    num_epochs = 30
    learning_rate = 0.001
    num_classes = 4
    ...
    if not os.path.exists(train_dir):
        ...
        print("  dataset_v1/train/target/")
        print("  dataset_v1/train/other_abnormal/")
        print()
        print("Or place images in dataset_v1/ with class subfolders and")
        print("this script will auto-split 80/20 into train/val.")
        return

    for cls_name in TARGET_CLASS_TO_IDX:
        cls_path = os.path.join(train_dir, cls_name)
        if not os.path.exists(cls_path):
            print(f"WARNING: Class folder '{cls_path}' not found. Creating empty folder.")
    ...
    print("=" * 56)
    print("  LabMind AI — V1 RBC Classifier Training")
    print("=" * 56)
    ...
    print()
    print("V1Provider will auto-detect this file on next Celery worker restart")
    print("and switch from 2-class fallback to full 4-class mode.")
